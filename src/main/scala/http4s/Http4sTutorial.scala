...
object Http4sTutorial extends IOApp {
  ...
  // The incoming request might not match with any of the available routes. Usually, such requests
  // should end up with 404 HTTP responses. This means that the return type can be expressed as
  // HttpApp, which maps Request[F] => F[Response[F]] i.e. the option has disappeared
